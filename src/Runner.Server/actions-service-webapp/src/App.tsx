        ...
        <Route path=":page/:owner/*" element={
          <Routes>
            ...
            <Route path=":page/:repo/*" element={
              <Routes>
                <Route path=":page" element={<GenericList id={(o: IWorkflowRun) => o.id} hasBack={true} summary={(o: IWorkflowRun) => <span>RunId: {o.id}<br/>Workflow: {o.fileName}</span>} url={(params) => `${ghHostApiUrl}/_apis/v1/Message/workflow/runs?owner=${encodeURIComponent(params.owner || "zero")}&repo=${encodeURIComponent(params.repo || "zero")}&page=${params.page || "0"}`}></GenericList>}/>
                <Route path="/" element={<Navigate to={"0"}/>}/>
