...
namespace GitHub.Runner.Common
{
    ...
    public sealed class HostContext : EventListener, IObserver<DiagnosticListener>, IObserver<KeyValuePair<string, object>>, IHostContext, IDisposable
    {
        ...
        public HostContext(string hostType, string logFile = null)
        {
            ...
            var refreshInternalInMS = 60 * 1000;
#if DEBUG
            // For L0, we will refresh faster
            if (!string.IsNullOrEmpty(Environment.GetEnvironmentVariable("_GITHUB_ACTION_SHORT_RESET_TIMER")))
            {
                refreshInternalInMS = int.Parse(Environment.GetEnvironmentVariable("_GITHUB_ACTION_SHORT_RESET_TIMER"));
            }
#endif
            _ = AuthMigrationAuthReenableAsync(TimeSpan.FromMilliseconds(refreshInternalInMS), _authMigrationAutoReenableTaskCancellationTokenSource.Token);
        }
        ...
        public void EnableAuthMigration(string trace)
        {
            ...
            AuthMigrationChanged(this, new AuthMigrationEventArgs(trace));
        }
