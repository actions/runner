...
namespace Microsoft.VisualStudio.Services.WebApi.Xml
{
    ...
    {
        // This method has a class constraint because we can't use reflection to mutate the properties of a struct without boxing
        public static void ReadDataMemberXml<T>(this XmlReader reader, T destination) where T : class
        {
            ...
            var serializableProperties = GetSerializableProperties(destination.GetType().GetTypeInfo()).MappedByName;

            while (reader.IsStartElement())
            {
                if (!serializableProperties.TryGetValue(reader.LocalName, out var property))
                {
                    reader.ReadOuterXml();
                    continue;
                }
                ...
                if (destination is GraphSubjectBase && property.SerializedName == nameof(GraphSubjectBase.Descriptor))
                {
                    var propertySerializer = GetSerializer(reader.NamespaceURI, reader.LocalName, typeof(SubjectDescriptor));
                    propertyValue = propertySerializer.Deserialize(reader.ReadSubtree()).ToString();
                }
                else
                {
                    var propertySerializer = GetSerializer(reader.NamespaceURI, reader.LocalName, property.SerializedType);
                    propertyValue = propertySerializer.Deserialize(reader.ReadSubtree());
                }
        ...
        public static void WriteDataMemberXml(this XmlWriter writer, object source)
        {
            ...
            var shouldWriteNamespace = writer.Settings == null;
            var rootNamespace = shouldWriteNamespace ? GetNamespace(type) : null;
            var serializableProperties = GetSerializableProperties(type).EnumeratedInOrder;
            foreach (var property in serializableProperties)
            {
                if (!property.ShouldSerialize(source))
                {
                    continue;
                }

                var propertyValue = property.GetValue(source);
                if (property.IsIgnorableDefaultValue(propertyValue))
                {
                    continue;
                }

                var propertySerializer = GetSerializer(rootNamespace, property.SerializedName, property.SerializedType);
                propertySerializer.Serialize(writer, propertyValue);

                if (!string.IsNullOrEmpty(property.SerializedNameForCamelCaseCompat))
                {
                    propertySerializer = GetSerializer(rootNamespace, property.SerializedNameForCamelCaseCompat, property.SerializedType);
                    propertySerializer.Serialize(writer, propertyValue);
                }
            }
        }
        ...
        {
            if (!NamespacesByType.TryGetValue(type, out string outputNamespace))
            {
                outputNamespace = $"http://schemas.datacontract.org/2004/07/{type.Namespace}";
                NamespacesByType.TryAdd(type, outputNamespace);
            }
