...
namespace GitHub.DistributedTask.Pipelines.Runtime
{
    ...
    public class PipelineAttemptBuilder
    {
            ...
            params Timeline[] timelines)
        {
            ...
            m_recordsByParent = new Dictionary<Guid, List<TimelineRecord>>();
        ...
        /// We will reuse previously successful stage attempts, and produce new attempts for 
        ///   failed stages, and any stages downstream from a failed stage. 
        /// This list of new attempts will be emitted as an output variable. 
        /// </summary>
        ...
        public Tuple<IList<StageAttempt>, IList<StageAttempt>> Retry()
        {
            ...
            // Walk the stage graph and schedule new attempts for failed stages + all downstream dependencies of
            // the failures. 
            GraphValidator.Traverse(this.Pipeline.Stages, (stage, dependencies) =>
            {
                ...
                {
                    throw new InvalidPipelineOperationException(PipelineStrings.InvalidRetryStageNeverRun(stage.Name));
                }
                else if (previousAttempt.Stage.State != PipelineState.Completed)
                {
                    throw new InvalidPipelineOperationException(PipelineStrings.InvalidRetryStageNotComplete(stage.Name));
                }

                var previousAttemptNeedsRetry = NeedsRetry(previousAttempt.Stage.Result);
                var upstreamDependencyNeedsRetry = dependencies.Any(x => stagesToRetry.Contains(x));
                if (previousAttemptNeedsRetry || upstreamDependencyNeedsRetry)
                {
                    stagesToRetry.Add(previousAttempt.Stage.Name);
                    var newAttempt = CreateAttempt(stage, previousAttempt, upstreamDependencyNeedsRetry);
            ...
            Boolean forceRetry = false)
        {
            ...
            GraphValidator.Traverse(stage.Phases, (phase, dependencies) =>
            {
                ...
                else
                {
                    ...
                    var phaseId = this.IdGenerator.GetPhaseInstanceId(newStageAttempt.Stage.Name, newPhaseAttempt.Phase.Name, newPhaseAttempt.Phase.Attempt);
                    ...
                    newStageAttempt.Timeline.Records.Add(CreateRecord(newPhaseAttempt.Phase, stageId, phaseId, phase.DisplayName ?? phase.Name, nameof(Phase), phaseOrder++, newPhaseAttempt.Phase.Identifier));
                    ...
                    if (previousAttemptNeedsRetry && !upstreamDependencyNeedsRetry)
                    {
                        ...
                        {
                            var previousJobId = this.IdGenerator.GetJobInstanceId(newStageAttempt.Stage.Name, newPhaseAttempt.Phase.Name, previousJobAttempt.Job.Name, previousJobAttempt.Job.Attempt);
                            if (NeedsRetry(previousJobAttempt.Job.Result))
                            {
                                ...
                                newJobAttempt.Job.Identifier = this.IdGenerator.GetJobIdentifier(newStageAttempt.Stage.Name, newPhaseAttempt.Phase.Name, newJobAttempt.Job.Name);
                                newPhaseAttempt.Jobs.Add(newJobAttempt);

                                // create a new record in "Pending" state based on the previous record. 
                                var newJobId = this.IdGenerator.GetJobInstanceId(newStageAttempt.Stage.Name, newPhaseAttempt.Phase.Name, newJobAttempt.Job.Name, newJobAttempt.Job.Attempt);
        ...
        {
            var stageRecords = new List<TimelineRecord>();
            foreach (var record in timeline.Records)
            {
                m_recordsById[record.Id] = record;
                if (record.ParentId != null)
                {
                    if (!m_recordsByParent.TryGetValue(record.ParentId.Value, out var childRecords))
                    {
                        childRecords = new List<TimelineRecord>();
                        m_recordsByParent.Add(record.ParentId.Value, childRecords);
                    }
        ...
        private void AddPhaseAttempts(
            ...
            IDictionary<Guid, List<TimelineRecord>> recordsByParent)
        {
        ...
        private void FixRecord(TimelineRecord record)
        {
        ...
        private IDictionary<Guid, List<TimelineRecord>> m_recordsByParent;
