...
namespace GitHub.DistributedTask.Expressions
{
    internal sealed class ConvertToJsonNode : FunctionNode
    {
        ...
        private void WriteMappingKey(
            ...
            Stack<ICollectionEnumerator> ancestors)
        {
            var str = PrefixValue(JsonUtility.ToString(key.ConvertToString(context)), ancestors, isMappingKey: true);
            memory.Add(str);
            writer.Append(str);
        }
            ...
            Stack<ICollectionEnumerator> ancestors)
        {
            ...
            switch (value.Kind)
            {
                ...
                case ValueKind.Version:
                    str = JsonUtility.ToString(value.ConvertToString(context));
                    break;
                ...
                case ValueKind.Number:
                    str = value.ConvertToString(context);
                    break;
        ...
        private sealed class ArrayEnumerator : ICollectionEnumerator
        {
            ...
            public Boolean MoveNext()
            {
                ...
                {
                    m_current = EvaluationResult.CreateIntermediateResult(m_context, m_enumerator.Current, out _);
                    m_index++;
                    return true;
                }
                else
        ...
        private sealed class ObjectEnumerator : ICollectionEnumerator
        {
            ...
            public Boolean MoveNext()
            {
                if (m_enumerator.MoveNext())
                {
                    var key = EvaluationResult.CreateIntermediateResult(m_context, m_enumerator.Current.Key, out _);
                    var value = EvaluationResult.CreateIntermediateResult(m_context, m_enumerator.Current.Value, out _);
