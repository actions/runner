    ...
    public sealed class PackagesTrimL0
    {
        ...
        public async Task RunnerLayoutParts_CheckExternalsHash()
        {
            using (TestHostContext hc = new TestHostContext(this))
            {
                ...
                Assert.True(string.Equals(hashResult, File.ReadAllText(externalsHashFile).Trim()), $"Hash mismatch for dotnet runtime. You might need to update `Misc/contentHash/externals/{BuildConstants.RunnerPackage.PackageName}` or check if `hashFiles.ts` ever changed recently.");
            }
        }
    }
}
