...
namespace GitHub.Runner.Common.Tests.Listener
{
    public sealed class RunnerL0 : IDisposable
    {
        ...
        [Trait("Level", "L0")]
        [Trait("Category", "Runner")]
        public async Task TestHostedSingleUseRunnerJobRequestMessageFromRunServiceExitsOnAcknowledgeJobNotFound()
        {
            using (var hc = new TestHostContext(this))
            {
                //Arrange
                Environment.SetEnvironmentVariable("ACTIONS_RUNNER_RETURN_JOB_RESULT_FOR_HOSTED", "true");
