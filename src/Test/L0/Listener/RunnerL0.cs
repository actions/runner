...
namespace GitHub.Runner.Common.Tests.Listener
{
    public sealed class RunnerL0 : IDisposable
    {
        ...
        [Trait("Level", "L0")]
        [Trait("Category", "Runner")]
        public async Task TestEphemeralRunnerJobRequestMessageFromRunServiceExitsOnJobAlreadyAcquired()
        {
            using (var hc = new TestHostContext(this))
            {
                //Arrange
                ...
                _runServer.Setup(x => x.GetJobMessageAsync("999", "github", It.IsAny<CancellationToken>()))
                    .ThrowsAsync(new TaskOrchestrationJobAlreadyAcquiredException("Job message already acquired '999'. job assignment is invalid: MissingKey"));
